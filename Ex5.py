...
for y in range(0 ,palavras_frase):
    ...
    for y1 in range(0, len(list_palavras[y])):
        print(len(list_palavras[y]))
        palavra_actual = list_palavras[y1]
        print(palavra_actual)
        print(palavra_actual[1])
        palavra_invert = palavra_actual[y1] + palavra_invert    # Aqui eu quero inverter o cada uma das palavras mas em ordem. COnsigo trocar a ordem das palavras, mas não estou a conseguir inverter a palavra em si
